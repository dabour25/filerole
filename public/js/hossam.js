...
function  edit_dates_modal(nights,id,book_from,book_to){
...
$('input[name="old_from_date"]').val(book_from);
$('input[name="old_date_to"]').val(book_to);
  var modal2 = document.getElementById('ChangeDatesModal2');
  modal2.style.display = "block";
...
function booking_canceled(){
  if (lang=='ar'){
    $('#booking_alerts').append('<div class="alert alert-success" style="background:green !important;color:#fff !important">لقد تم الغاء الحجز</div>')
  }
  else{
    $('#booking_alerts').append('<div class="alert alert-success" style="background:green !important;color:#fff !important">booking has been cancelled</div>')
  }
  location.reload();
}
function checked_in_done(){
  if (lang=='ar'){
    $('#booking_alerts').append('<div class="alert alert-success" style="background:green !important;color:#fff !important">لقد تم دخول العميل بنجاح</div>')
  }
  else{
    $('#booking_alerts').append('<div class="alert alert-success" style="background:green !important;color:#fff !important">customer has been checked in successfully</div>')
  }
  location.reload();
}
...
window.location.pathname = '/admin/booking/'+book_id+'/invoice';
  ...
  $.ajax({
    ...
    success: function(data) {
          if(data.data=="succeede"){
            ...
            $('input[name="cancel_book_id"]').val(book_id);
            $('input[name="cancel_nights"]').val(data.nights);
            $('input[name="cancel_book_from"]').val(data.book_from);
            $('input[name="cancel_book_to"]').val(data.book_to);

            if(data.pay_method==0){
              $('#cancel_pay_method').empty();
            }
            $('#cancel_reasons').empty();
            $.each(data.rooms, function(key, value) {
              $('#rooms_policy').append('<div class="col-xs-12">'+data.rooms[key].name+'   ------------------------------------------------   '+data.rooms[key].policy+'</div>');

            });

            if (lang=='ar'){
              $('#total_policy').append('<div class="col-md-12">الاجمالي----------------------------------------------------------------'+data.total_charge+'</div>');
            }
            else{
              $('#total_policy').append('<div class="col-md-12">Total--------------------------------------------------------'+data.total_charge+'</div>');
            }
            var j=0;
            $.each(data.cancel_reasons, function(key, value) {
              if(j==0){
                $('#cancel_reasons').append('<input type="radio" name="reason"  checked style="height:25px; width:30px; vertical-align: middle;" value="'+value+'"> '+key+'<br><br>');
              }
              else{
                $('#cancel_reasons').append('<input type="radio" name="reason" style="height:25px; width:30px; vertical-align: middle;" value="'+value+'"> '+key+'<br><br>');
              }

             j++;
            });

            var modal2 = document.getElementById('CancelBooking');
            modal2.style.display = "block";
...
function close_cancel_booking(){
  var modal2 = document.getElementById('CancelBooking');
  modal2.style.display = "none";
}

function confirm_cancel_booking(){
var book_id=$('input[name="cancel_book_id"]').val();

var cancel_reason_id=$('input[name="reason"]').val();
var payment_method=$('select[name="cancel_payment_method"]').val();
  ...
  $.ajax({
    ...
    success: function(data) {
          if(data.data=="succeede"){
  setTimeout(booking_canceled(), 200000);
...
function cancel_edit_dates(){
  var nights=parseInt($('input[name="cancel_nights"]').val());
  var id=$('input[name="cancel_book_id"]').val();
  var book_from=$('input[name="cancel_book_from"]').val();
  var book_to=  $('input[name="cancel_book_to"]').val();
   close_cancel_booking();
edit_dates_modal(nights,id,book_from,book_to);
}

function checkin_booking(book_id){
  $.ajaxSetup({
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    }
  ...
  $.ajax({
    ...
    success: function(data) {
      if(data.data=='succeede'){
        ...
        $('input[name="reserv_no"]').val(data.booking.confirmation_no);
        $('input[name="customer_name"]').val(data.booking.cust_name);
        $('input[name="adults_no"]').val(data.booking.adult_no);
        $('input[name="child_no"]').val(data.booking.chiled_no);
        $('input[name="nights"]').val(data.booking.nights);
        $('input[name="check_in_time"]').val(data.booking.book_from);
        $('input[name="check_out_time"]').val(data.booking.book_to);
        $('input[name="check_in_book_id"]').val(data.booking.id);
        $('input[name="check_in_cust_id"]').val(data.booking.cust_id);
        $('input[name="checkin_category_num_ids[]"]').val(data.room_numbers);
        $('input[name="code"]').val(data.code);
        if(data.code==0){
          $('#customer_verification_message').empty();
          $('#customer_verification_message').append('<br><br><div class="badge badge-danger">'+data.message+'</div>')
        }
        $('#customer_verification_message').append('')
        $('#room_numbers').empty();
        $.each(data.room_numbers, function(key, value) {
          $('#room_numbers').append('<div class="col-md-3"><i class="fa fa-key" aria-hidden="true"></i><span class="label label-primary">'+value+'</span></div>');

            });
        var modal2 = document.getElementById('CheckinBooking');
        modal2.style.display = "block";
...
function close_checkin_booking(){
  var modal2 = document.getElementById('CheckinBooking');
  modal2.style.display = "none";
}
function check_customer_data(){
 var cust_id=$('input[name="check_in_cust_id"]').val();
 $.ajax({
   url: '/admin/check_in_customer_data',
   dataType: 'json',
   data:{
       cust_id:cust_id,
     },
   ...
   success: function(data) {
     var modal2 = document.getElementById('CheckinBooking');
     modal2.style.display = "none";
     if(data.customer.person_idtype!=null){
       $('select[name="person_type_id"]').val(data.customer.person_idtype);
     }
     $('input[name="verify_cust_id"]').val(data.customer.id);
     $('input[name="personal_id"]').val(data.customer.person_id);
     $('#customer_image').empty();
     $('#customer_image').append('<br><br><img src="'+data.customer.image_verify+'" style="width: 300px;height: 200px;" id="set2">');

     var modal2 = document.getElementById('CheckinCustomer');
     modal2.style.display = "block";
...
function close_checkin_customer(){
  var modal2 = document.getElementById('CheckinCustomer');
      modal2.style.display = "none";
}
...
function save_customer_data(){
  ...
  if($('input[name="personal_id"]').val()=='' && $('input[name="person_image"]').val()==''){
    $('#verify-id').empty();
    if(lang=='ar'){
      $('#verify-id').append('<br><br><div class="badge badge-danger">يجب ادخال رقم البطاقة او صورتها علي الاقل</div>');
    }
    else{
      $('#verify-id').append('<br><br><div class="badge badge-danger">you should enter id number or apicture of it at least</div>');
    }

  }
  else{
    $('#verify-id').empty();
    $('#customer_personal_id').ajaxSubmit({

        url: siteUrl + '/admin/save_customer_id_data', type: 'post',
        beforeSubmit: function (response) {
      },
        success: function (response) {
        $('#customer_verification_message').empty();
        $()
        close_checkin_customer();
        if(lang=="ar"){
          $('#customer_verification_message').append('<br><br><div class="badge badge-primary">لقد تم التحقيق من الشخصية بنجاح </div>')
        }
        else{
          $('#customer_verification_message').append('<br><br><div class="badge badge-primary">you have been check customer successfully</div>')
        }
...
function check_in_done(){
  ...
  location.reload();
    ...
    function check_out_done(){
    if (lang=='ar'){
      $('#booking_alerts').append('<div class="alert alert-success" style="background:green !important;color:#fff !important">لقد تم تسجيل خروج العميل بنجاح</div>')
    }
    else{
      $('#booking_alerts').append('<div class="alert alert-success" style="background:green !important;color:#fff !important">customer has been checked out successfully</div>')
    }
    location.reload();
    }
...
function check_in_customer(){


  var rooms_numbers=[];
  $('input[name="checkin_category_num_ids[]"]').each(function(){
        rooms_numbers.push(this.value);

    });
  var book_id=$('input[name="check_in_book_id"]').val();
  var code=$('input[name="code"]').val();
  var check_in_date=new Date($('input[name="check_in_time"]').val());
  var check_out_time=new Date($('input[name="check_out_time"]').val());
  var current_date=new Date($('input[name="current_time"]').val());
  var validation =0;
  if(lang=='ar'){
    var code_message='يجب ملاحظة ان هذا العميل ليس له اي بطاقة هوية';
    var check_in_message='لا يمكن دخول العميل قبل موعد الحجز';
    var checkout_message='لا يمكن دخول العميل بعد موعد الحجز';
  }
  else{
    var code_message='you should notice that customer does not have any credentials';
    var check_in_message='customer can not check in before reservation date';
    var checkout_message='customer can not check in after reservation date';
  }
  if(code==0){
    $('#customer_verification_message').empty();

    $('#customer_verification_message').append('<br><br><div class="badge badge-danger">'+code_message+'</div>');
    validation=1;

  }
  if(current_date.getTime()<check_in_date.getTime()){
    $('#customer_verification_message').empty();

    $('#customer_verification_message').append('<br><br><div class="badge badge-danger">'+check_in_message+'</div>');
    validation=1;
  }

  if(current_date.getTime()>check_out_time.getTime()){
    $('#customer_verification_message').empty();

    $('#customer_verification_message').append('<br><br><div class="badge badge-danger">'+checkout_message+'</div>');
    validation=1;
  }
  if(validation==0){
    $.ajaxSetup({
    ...
    $.ajax({
      url: '/admin/check_in_booking',
      dataType: 'json',
      data:{
          book_id:book_id,
          room_numbers:rooms_numbers,

        },
      type: 'POST',
      success: function(data) {

      close_checkin_booking();
         setTimeout(check_in_done(), 200000);
...
function print_invoice(request_id){
  window.open('/admin/transactions/'+request_id+'/print','_blank');
  }
  
  function checkout_booking(book_id){
  ...
  $.ajax({
    url: '/admin/checkout_check_invoice',
    dataType: 'json',
    data:{
       book_id:book_id
      ...
      if(data.data=="no_invoices"){
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            setTimeout(check_out_done(), 200000);
        ...
        $('input[name="total_value"]').val(data.total);

        var requests_ids=[];
        var remaining_amounts=[];

        var booked_rooms='';
        $("#xtreme-table4 tbody").empty();
        $.each(data.invoices, function (key) {
             requests_ids.push(data.invoices[key].id);
             remaining_amounts.push(data.invoices[key].remaining);
            booked_rooms+='<tr>';
            booked_rooms+='<td>'+data.invoices[key].invoice_no+'</td>';
            booked_rooms+='<td>'+data.invoices[key].required_amount+'</td>';
            booked_rooms+='<td>'+data.invoices[key].paid_amount+'</td>';
            booked_rooms+='<td>'+data.invoices[key].remaining+'</td>';
            if(lang=='ar'){
              booked_rooms+='<td><button type="button" class="btn btn-primary" onclick="pay_invoice('+data.invoices[key].id+','+data.invoices[key].remaining+')">دفع</button></td>';
              if(data.invoices[key].invoice!=null){
                booked_rooms+='<td><button type="button" class="btn btn-info" onclick="print_invoice('+data.invoices[key].id+')">فاتورة</button></td>';
              }

            }
            else{
              booked_rooms+='<td><button type="button" class="btn btn-primary" onclick="pay_invoice('+data.invoices[key].id+','+data.invoices[key].remaining+')">Pay</button></td>';
              if(data.invoices[key].invoice!=null){
                booked_rooms+='<td><button type="button" class="btn btn-info" onclick="print_invoice('+data.invoices[key].id+')">Invoice</button></td>';
              }
        ...
        $('input[name="requests_head_ids[]"]').val(requests_ids);
        $('input[name="remaining_amounts[]"]').val(remaining_amounts);
        $('input[name="check_out_book_id"]').val(book_id);
        $('input[name="check_out_customer_id"]').val(data.customer_id);
        $("#xtreme-table4 tbody").append(booked_rooms);

        var modal2 = document.getElementById('CheckoutCustomer');
            modal2.style.display = "block";
      }
...
function close_checkout_customer(){
  var modal2 = document.getElementById('CheckoutCustomer');
      modal2.style.display = "none";
}
function pay_invoice(request_head_id,amount){

  var book_id=$('input[name="check_out_book_id"]').val();
  var remaining=$('input[name="total_value"]').val();
  var customer_id=$('input[name="check_out_customer_id"]').val();
  $('input[name="check_amount"]').val(amount);
  $('input[name="check_out_remaining"]').val(remaining);
  $('input[name="check_out_request_head_id"]').val(request_head_id);
  $('input[name="check_out_remaining"]').val()
  $('input[name="check_out_pay_book_id"]').val(book_id);
  $('input[name=check_out_pay_method]').val(0);
  $('input[name="check_out_pay_customer_id"]').val(customer_id);
   close_checkout_customer()
    var modal2 = document.getElementById('CheckoutPay');
        modal2.style.display = "block";
}
  function check_out_pay_all(){
    var amount=$('input[name="total_value"]').val();
    var request_head_id=$('input[name="requests_head_ids[]"]').val();
    var book_id=$('input[name="check_out_book_id"]').val();
    var customer_id=$('input[name="check_out_customer_id"]').val();
    var remaining_amounts=$('input[name="remaining_amounts[]"]').val();
    $('input[name="check_amount"]').val(amount);
    $('input[name="check_out_remaining"]').val(amount);
    $('input[name="check_out_request_head_ids[]"]').val(request_head_id);
    $('input[name="check_out_pay_book_id"]').val(book_id)
    $('input[name=check_out_pay_method]').val(1);
    $('input[name="check_out_pay_customer_id"]').val(customer_id);
    $('input[name="checkout_remaining_amounts[]"]').val(remaining_amounts);
    close_checkout_customer()
    var modal2 = document.getElementById('CheckoutPay');
        modal2.style.display = "block";
  }
  function close_check_out_pay(){
    var modal2 = document.getElementById('CheckoutPay');
        modal2.style.display = "none";
  }
...
function check_out_pay(){
  var book_id=$('input[name="check_out_pay_book_id"]').val();
  var amount=$('input[name="check_amount"]').val();
  var remaining=  $('input[name="check_out_remaining"]').val();
  var request_id=$('input[name="check_out_request_head_id"]').val();
  var request_ids=$('input[name="check_out_request_head_ids[]"]').val();
  var type=$('input[name=check_out_pay_method]').val();
  var customer=$('input[name="check_out_pay_customer_id"]').val();
  var bank_id=$('select[name="check_out_payment_method"]').val();
  var remaining_amounts=$('input[name="checkout_remaining_amounts[]"]').val();
  ...
  $.ajax({
    url: '/admin/checkout_pay_invoice',
    dataType: 'json',
    data:{
       book_id:book_id,
       amount:amount,
...
function readURL_checkin(input) {
