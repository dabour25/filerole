...
/**
 * Returns a formatted price using the current currency global variables using Intl JS API.
 * In case of an error : Returns price formatted using old method (format_price) .
 * @param {string} price - price you wish to format.
 * @param {boolean} symbol - whenther to return currency symbol or not
 */
function format_price_super_simple(price, symbol = false){
	// currently forced to arabic numbers (1234567890) Not Hindi Numbers at all.
	var locale = 'en-' + country_code;
	try {
		if(!symbol) return new Intl.NumberFormat(locale, { style: 'decimal', currency_code }).format(price);
		return new Intl.NumberFormat(locale, { style: 'currency', currency: currency_code }).format(price);
	} catch (error){
...
function createBreadCrumbs(breadCrumbs)
{
	...
	for(i=0;i < breadCrumbs.length; i++)
	{
		...
		if(breadCrumb['link'] != '')
		{
			linkText = ` href="${breadCrumb['link']}" `;
		}
		if(i == breadCrumbs.length - 1)
		{
            breadText += `<li ${liAttributesText} class='breadcrumb-item active'><span>${breadCrumb['title']}</span></li>`
		}else{
            breadText += `<li ${liAttributesText} class='breadcrumb-item' ><a ${aAttributesText} ${linkText} >${breadCrumb['title']}</a></li>`;
		}
	}
