...
function fillData(bookingData)
{
    for (i = 0; i < bookingData.length; i++)
    {
        ...
        invoiceItem = bookingData[i];
        $('.service-input-' + invoiceItem).click();
    }
...
id=$('input[name="reservation_id"]').val();
$.get('/admin/reservation_details/'+id, function (data) {
    if(data)
    {
      $('#client_selectpicker').val(data['cust_id']);

      $('#client_selectpicker').trigger('change');

      $('.staff-item-'+data['captin_id']).click();
    fillData(data['data']);
