        ...
        <div class="row">
            <form method="post" action="{{route('roles.store')}}" enctype="multipart/form-data">
                    ...
                    <div class="check_all">
                      ...
                      <ul class="Newnav">
                        ...
                        <li class="dropdown mega-dropdown">
                            <input type="checkbox" onclick="checkallin({{$par->id}})" class="checkhour" id="par{{$par->id}}" name="acs[]" value="{{$par->id}}">
                            ...
                            <ul class="dropdown-menu dropdown-menuNew mega-dropdown-menu">
                                ...
                                <li class="check_menu">
                                   ...
                                   <input type="checkbox" name="acs[]" type="checkbox" value="{{$auth->id}}" class="checkhour checkchild{{$par->id}}" id="child{{$auth->id}}" onclick="checktiny({{$par->id}},{{$auth->id}})">
                                   <span class="checkmark"></span>
                                    ...
                                    @foreach($auth->childs()->where('org_id',Auth::user()->org_id)->orderby('porder')->get() as $child)
                                        ...
                                        <input type="checkbox" name="acs[]" type="checkbox" value="{{$child->id}}" class="checkhour checkchild{{$par->id}} tinycheck{{$auth->id}}" id="tiny{{$child->id}}" onclick="checkselected({{$par->id}},{{$auth->id}})"> 
                                        <span class="checkmark"></span>
                                        </label>
                                     @endforeach
                                     </div>
                ...
                <div class="col-md-12 form-group{{$errors->has('description') ? ' has-error' : ''}}">
                    ...
                    <textarea type="text" class="form-control textall" name="description">{{old('description')}}</textarea>
