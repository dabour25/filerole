                            ...
                            <div class="col-md-12 form-group">
                                <div class="check_all">
                                  ...
                                  <ul class="Newnav">
                                     ...
                                     @foreach( $parent_auths as $parent_auth)
                                      
                                    <li class="dropdown mega-dropdown">
                                        <input type="checkbox" onclick="checkallin()">
                                        <a href="javascript:;" class="dropdown-toggle" tabindex="-1" aria-expanded="false" class="dropdown-toggle">
                                          <p> {{app()->getLocale() == 'ar' ? $parent_auth->funcname : $parent_auth->funcname_en }} <i class="fas fa-chevron-down"></i></p>
                                        </a>
                                        
                                        <ul class="dropdown-menu dropdown-menuNew mega-dropdown-menu">
                                            @foreach( $parent_auth->childs()->where('org_id', Auth::user()->org_id)->where('funcparent_id','>',0)->orderBy('porder')->get()   as $auth)
                                            <li class="check_menu">
                                                
                                                <label class="containerss">
                                                {{app()->getLocale() == 'ar' ? $auth->funcname : $auth->funcname_en }}
                                               <input type="checkbox"  type="checkbox" value="{{$auth->id}}" class="checkhour" {{ App\functions_role::where('role_id' ,$role->id)->where('org_id',Auth::user()->org_id)->where('functions_id',$auth->function_id)->count()>0 ? 'checked'  : ''  }}  name="acs[]" >
                                               <span class="checkmark"></span>
                                                ...
                                                @foreach($auth->childs()->where('org_id',Auth::user()->org_id)->orderby('porder')->get() as $child)
                                                    ...
                                                    <input type="checkbox" type="checkbox"  class="checkhour" {{ App\functions_role::where('role_id' ,$role->id)->where('org_id',Auth::user()->org_id)->where('functions_id',$auth->function_id)->count()>0 ? 'checked'  : ''  }}  value="{{$auth->id}}"  name="acs[]"> 
                                                    <span class="checkmark"></span>
                                                    </label>
                                                 @endforeach
                                                 </div>
