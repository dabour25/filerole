		...
		<div class="patern-layer-two" style="background-image: url(https://expert-themes.com/html/tecno/images/icons/banner-icon-2.png)"></div>
           <div class="owl-carousel owl-theme" id="sliderHome">
             @if(count($offers))
                ...
                @if(front_cat_price($offer->cat_id,$org_id_login)['original_price'] != front_cat_price($offer->cat_id,$org_id_login)['catPrice'])
               <div class="item">
                   ...
                   <div class="slider-text">
                       ...
                       <div class="box-text wow fadeInDown" data-wow-delay="0.50s">
                           <div class="inner_box">
                               ...
                               <p class="price wow fadeIn" data-wow-delay="0.5s">{{front_cat_price($offer->cat_id,$org_id_login)['catPrice']}}</p>
                               <p class="old_price wow fadeIn" data-wow-delay="0.5s">{{front_cat_price($offer->cat_id,$org_id_login)['original_price']}}</p>
                                  @if($show_cart)
                                    <a href="#" data-name="{{app()->getLocale() =='ar' ? $offer->name : $offer->name_en}}" data-price="{{front_cat_price($offer->cat_id,$org_id_login)['catPrice']}}" data-id="{{ $offer->cat_id }}" data-description="{{app()->getLocale() =='ar' ?$offer->description :$offer->description_en}}" class="add-to-cart btn btn-primary">
        ...
        <div class="container">
            <div class="all_title">
             ...
             $labels=\App\ActivityLabelSetup::where(['type'=>item_service,'activity_id'=>$org->activity_id ])->first();
                ...
                <p class="wow fadeInUp" data-wow-delay="0.20s">Our latest product of 	{{ app()->getLocale() == 'ar' ? $org->name : $org->name_en}}..</p>
            </div>
                    ...
                    <div class="col-md-3 col-sm-6 col-xs-6">
                     <div class="item_pro wow fadeIn" data-wow-delay="0.20s">
                       ...
                       <div class="price_add">
                             ...
                             @if(front_cat_price($last_prodct->id,$org_id_login)['catPrice'] != front_cat_price($last_prodct->id,$org_id_login)['original_price'] )

                             <h6>{{ front_cat_price($last_prodct->id,$org_id_login)['catPrice'] }}</h6>
                             <h6 class="old_price wow fadeIn">{{ front_cat_price($last_prodct->id,$org_id_login)['original_price']}}</h6>
                              @if($show_cart)
                             <a href="#" data-name="{{app()->getLocale() =='ar' ? $last_prodct->name : $last_prodct->name_en}}" data-price="{{ front_cat_price($last_prodct->id,$org_id_login)['catPrice']}}"  data-id="{{ $last_prodct->id}}" data-description="{{app()->getLocale() =='ar' ? $last_prodct->description : $last_prodct->description_en}}" class="add-to-cart btn btn-primary"><i class="fas fa-cart-plus"></i>add to cart </a>
                             @endif
                             @else
                             <h6>{{ front_cat_price($last_prodct->id,$org_id_login)['original_price']}}</h6>
                              @if($show_cart &&(front_cat_price($last_prodct->id,$org_id_login)['original_price']) )
                             <a href="#" data-name="{{app()->getLocale() =='ar' ? $last_prodct->name : $last_prodct->name_en}}" data-price="{{ front_cat_price($last_prodct->id,$org_id_login)['original_price']}}"  data-id="{{ $last_prodct->id}}" data-description="{{app()->getLocale() =='ar' ? $last_prodct->description : $last_prodct->description_en}}" class="add-to-cart btn btn-primary"><i class="fas fa-cart-plus"></i>add to cart </a>
                             @endif
                             @endif
                    ...
                    <div class="item">
                        <div class="item_pro wow fadeIn" data-wow-delay="0.20s">
                           <a href="details/{{ $best_seler->id }}"><img src="{{asset(str_replace(' ', '%20', \App\Photo::find($best_seler->photo_id)->file))}}"></a>
                            ...
                            @if(front_cat_price($best_seler->id,$org_id_login)['original_price'] != front_cat_price($best_seler->id,$org_id_login)['catPrice'])
                            <div class="price_add">
                                <h6>{{ front_cat_price($best_seler->id,$org_id_login)['catPrice'] }}</h6>
                                <h6>{{ front_cat_price($best_seler->id,$org_id_login)['original_price'] }}</h6>
                                   @if($show_cart)
                                <a href="#" data-name="{{app()->getLocale() =='ar' ? $best_seler->name :
                                    $best_seler->name_en}}" data-price="{{front_cat_price($best_seler->id,$org_id_login)['catPrice']}}"  data-id="{{ $best_seler->id}}" data-description="{{app()->getLocale() =='ar' ? $best_seler->description : $best_seler->description_en}}" class="add-to-cart btn btn-primary">  <i class="fas fa-cart-plus"></i>add to cart </a>
                                @endif
                            </div>
                            @else
                            <div class="price_add">
                                 ...
                                 @if($show_cart)
                                 <a href="#" data-name="{{app()->getLocale() =='ar' ? $best_seler->name :
                                     $best_seler->name_en}}" data-price="{{ front_cat_price($best_seler->id,$org_id_login)['original_price']}}"  data-id="{{ $best_seler->id}}" data-description="{{app()->getLocale() =='ar' ? $best_seler->description : $best_seler->description_en}}" class="add-to-cart btn btn-primary">  <i class="fas fa-cart-plus"></i>add to cart </a>
                                 @endif
                            </div>
                            @endif
                        </div>
