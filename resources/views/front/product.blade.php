        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-md-7 col-xs-12">
                    ...
                    @if(front_cat_price($category_price->id,$org_id_login)['catPrice'] == front_cat_price($category_price->id,$org_id_login)['original_price'] )
                      <p class="price">{{front_cat_price($category_price->id,$org_id_login)['original_price']}}</p>
                       @if($show_cart  && (front_cat_price($category_price->id,$org_id_login)['catPrice']) )

                    <a href="#" data-name="{{app()->getLocale() =='ar' ? $category_price->name : $category_price->name_en}}"  data-price="{{front_cat_price($category_price->id,$org_id_login)['catPrice']}}" data-id="{{$category_price->id}}"  data-description="{{app()->getLocale() =='ar' ?$category_price->description : $category_price>description_en}}" class="add-to-cart btn btn-primary"><i class="fas fa-cart-plus"></i>add to cart </a>

                      @endif
                       @else
                        <p class="old_price">{{front_cat_price($category_price->id,$org_id_login)['original_price']}}</p>
                        </br>
                        <p class="price">{{front_cat_price($category_price->id,$org_id_login)['catPrice']}}</p>
                          @if($show_cart)
                        <a href="#" data-name="{{app()->getLocale() =='ar' ? $category_price->name :$category_price->name_en}}" data-price="{{front_cat_price($category_price->id,$org_id_login)['catPrice']}}"  data-id="{{ $category_price->id}}" data-description="{{app()->getLocale() =='ar' ? $category_price->description :$category_price->description_en}}"  class="add-to-cart btn btn-primary" ><i class="fas fa-cart-plus"></i>add to cart </a>
                         @endif
                        @endif
