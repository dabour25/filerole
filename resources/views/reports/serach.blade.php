...
@endphp
    ...
    $extranal=App\externalReq::where(['cust_id' => $value->id])->where ('request_dt','>=',app('request')->input('report_date_from'))->where('request_dt','<=',app('request')->input('report_date_to')) ->get();
    ...
    $extranal=App\externalReq::where(['cust_id' => $value->id])->where('request_dt','<=',app('request')->input('report_date_to'))->get();
    ...
    $extranal=App\externalReq::where(['cust_id' => $value->id])->where('request_dt','>=',app('request')->input('report_date_from'))->get();



	}elseif(app('request')->input('search_name') ){
		$head = App\CustomerHead::where(['cust_id'   => $value->id ])->get();
    $extranal=App\externalReq::where(['cust_id' => $value->id])->get();
        ...
        $extranal=App\externalReq::where(['cust_id' => $value->id, 'request_dt' => date('Y-m-d')])->get();
		   ...
		   @php
			...
			<tr>
			  ...
			  @endphp
               ...
               @if( $due_date > date('d-m-Y'))<span class="label label-danger" style="font-size:12px;">@lang('strings.transactions_status_5')</span>
  ...
  <table id="xtreme-table" class="display table" style="width: 100%; cellspacing: 0;">
    ...
    <tbody>
      @foreach($extranal as $value)


    <tr>
 @php
    $total=DB::table('external_trans')->where('external_req_id',$value->id )->sum(DB::raw('quantity * final_price'));
    $total_etra_request +=$total;
    $total_fees +=$value->delivery_fees;
@endphp        
    <td>{{$value->id}}</td>
    <td>{{$value->invoice_no}}</td>
    <td>{{$total}}</td>
   <td>{{$value->delivery_fees}}</td>
    @if($value->confirm =='p')  <td> <span class="label label-info"style="font-size:9px;">  بالسلة  </span></td>
    @elseif($value->confirm =='x') <td> <span class="label label-success" style="font-size:9px;">   مدفوعة اولاين   </span></td>
    @elseif($value->confirm =='y')<td> <span class="label label-success" style="font-size:9px;"> مؤكدة</span></td>
    @elseif($value->confirm =='c')<td> <span class="label label-success" style="font-size:9px;">ملغى</span></td>
    @elseif($value->confirm =='d')<td>  <span class="label label-success" style="font-size:9px;"> منفذ</span></td>
    @elseif($value->confirm =='n')<td>  <span class="label label-warning" style="font-size:9px;">قيد الانتظار </span></td>
    @elseif($value->confirm =='yx')<td> <span style="width:150px;" class="label label-success" style="font-size:9px;">مدفوعة اولاين وموكدة لتوصيل</span></td>
    @endif
      </tr>
    @endforeach
	...
	<tfoot>
                                        <tr>
                                            <td>{{ __('strings.Total') }}</td>
                                            <td></td>
                                            <td>{{ Decimalplace(abs($total_etra_request)) }}</td>
                                            <td>{{ Decimalplace($total_fees) }}</td>
                                            <td></td>
                                        </tr>
                                        </tfoot>
