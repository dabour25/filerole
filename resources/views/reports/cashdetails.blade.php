...
$users3=App\User::where(['users.org_id' => Auth::user()->org_id, 'is_active'=>1  ])->select('name','users.id','name_en')->get();

@endphp

		@foreach($users3 as $role)
         ...
         @endforeach
                        ...
                        @endphp
                                                <table id="xtreme-table" class="display table" style="width: 100%; cellspacing: 0;">
                                                    ...
                                                    @php
                                                    $total_extra_pay +=abs($user->deail->extra_paid1);
                                                    @endphp
                                                     <tr>
                                                       <td>{{ Dateformat($user->deail->request_dt)}}</td>
                                                      {{$user->deail->extra_paid}} <td>{{abs($user->deail->extra_paid1)}}</td>
