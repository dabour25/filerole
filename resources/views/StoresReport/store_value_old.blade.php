                                    ...
                                    <tbody>
                                        @foreach($transactions = App\Transactions::where(['store_id' => $value->id])->groupBy('cat_id')->get() as $v2)
                                            @php
                                                ...
                                                $final_price = App\PriceList::where(['cat_id' => $v2->cat_id, 'active' => 1,'org_id' => Auth::user()->org_id ])->where('date', '<', date('Y-m-d'))->orderBy('date', 'desc')->value('final_price');
                                            ...
                                            @if($category->type->type == 1 || $category->type->type == 4)
                                                <tr>
                                                    ...
                                                    <td>{{ $category->name }}</td>
                                                    <td>{{ $quantity }}</td>
                                                    <td>{{ round($final_price, 2) }}</td>
                                                    <td>{{ round($AveragePrice, 2) }}</td>
                                                    <td>{{ round($TotalSale, 2) }}</td>
                                                    <td>{{ round($quantity * $TotalPurchase, 2) }}</td>
                                                    <td>{{ round($TotalSale - ($quantity * $TotalPurchase), 2) }}</td>
                                                </tr>
                                            @endif
                                        @endforeach
                                    </tbody>
