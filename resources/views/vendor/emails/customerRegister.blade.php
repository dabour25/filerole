...
<body>
    ...
    <div class="email">
        <div class="top_hi">
            <h2> <span>{{ app()->getLocale()=='ar'? $data['org']->name : $data['org']->name_en  }}</span> لإدارة الأعمال </h2>

        </div>

        <div class="contant_mail">
            <h6>@lang('strings.welcome_to') <span>{{$data['customer_name']}}</span> </h6>
						<p>{{ app()->getLocale()=='ar'? $data['org']->about_us : $data['org']->aboutus_en  }}</p>
        </div>
        ...
        <div class="need-help">
            ...
            <p> إذا كان لديك إستسفار أو وجاهتك مشكلة ما أثناء إستخدام برنامج فايل رول راسلنا علي
                <a href="mailto:{{$data['org']->email}}">{{$data['org']->email}}</a>
