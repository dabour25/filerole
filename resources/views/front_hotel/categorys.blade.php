...
@section('content')
    ...
    <div id="page_header" class="page-subheader page-subheader-bg">
      ...
      $cats=\App\ActivityLabelSetup::where(['type'=>items,'activity_id'=>$org->activity_id ])->first();
      $sers=\App\ActivityLabelSetup::where(['type'=>services,'activity_id'=>$org->activity_id ])->first();
                        ...
                        <div class="col-md-9 col-xs-12">
                            ...
                            <div class="tab-content">
                                ...
                                <div class="tab-pane active" id="hidden_tab" role="tabpanel" aria-labelledby="hidden_tab">
                                 ...
                                 $offers=\App\Offers::where('active',1)->where('org_id',$org->id)->where('cat_id',$product->product_type->id)->whereDate('date_from','<=' , date('Y-m-d'))->whereDate('date_to','>=' , date('Y-m-d'))->orderBy('created_at','desc')->first();
                                 $prices=App\PriceList::where('active',1)->where('org_id',$org->id)->whereDate('date','<=',date('Y-m-d'))->where('cat_id',$product->product_type->id)->orderBy('date','desc')->first();
                                ...
                                <div class="item_pro ">
                                <a href="details/{{ $product->product_type->id }}"> <img src=" {{asset(str_replace(' ', '%20', \App\Photo::find($product->product_type->photo_id)->file))}}"></a>
                                 <h3>{{app()->getLocale() =='ar' ? $product->product_type->name : $product->product_type->name_en}}</h3>
                                 @if($offers)

                                 <div class="price_add">
                                     <h6 class="final_price">{{Decimalpoint($product_offers->final_price) }}</h6>
                                     <h6>{{Decimalpoint($product_offers->offer_price)}}</h6>
                                       @if($show_cart)
                                 ...
                                 @else
                                 <div class="price_add">
                                     <h6>{{Decimalpoint($product->product_type->final_price) }}</h6>
                                       @if($show_cart)
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-9 col-xs-12">
                            ...
                            <div class="tab-content">
                                ...
                                $offers=\App\Offers::where('active',1)->where('org_id',$org->id)->where('cat_id', $service->service_type->id)->whereDate('date_from','<=' , date('Y-m-d'))->whereDate('date_to','>=' , date('Y-m-d'))->orderBy('created_at','desc')->first();
                                $prices=App\PriceList::where('active',1)->where('org_id',$org->id)->whereDate('date','<=',date('Y-m-d'))->where('cat_id',$service->service_type->id)->orderBy('date','desc')->first();

                                if($offers)
                                $service_offers=\APP\Category::join('offers', 'offers.cat_id', '=', 'categories.id')
                                ->join('price_list', 'price_list.cat_id', '=', 'categories.id')
                                ->where([ 'categories.active' => 1,'categories.category_type_id' =>$service->id ,'categories.org_id'=>$org->id])
                                ->where('offers.id', $offers->id)
                                ->where('price_list.id', $prices->id)
                                ->select('categories.*','offer_price', 'final_price')
                                ->first();
                                @endphp
                                  <div class="col-md-4 col-xs-12">
                                   <div class="item_pro ">
                                  <a href="details/{{$service->service_type->id }}"> <img src="{{asset(str_replace(' ', '%20', \App\Photo::find($service->service_type->photo_id)->file))}}"></a>

                                  <h3>{{app()->getLocale() =='ar' ? $service->service_type->name : $service->service_type->name_en}}</h3>
                              @if($offers)
                                <div class="price_add">
                                    <h6 class="final_price">{{Decimalpoint( $service_offers->final_price)}}</h6>
                                    <h6>{{Decimalpoint($service_offers->offer_price)}}</h6>
                                      @if($show_cart)
                                ...
                                <div class="price_add">
                                    <h6>{{Decimalpoint($service->service_type->final_price)}}</h6>
                                      @if($show_cart)
