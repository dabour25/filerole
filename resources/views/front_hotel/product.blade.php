        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-md-5 col-xs-12">
                    <div class="img_pro">
                        <img class="main_image" src="{{asset(str_replace(' ', '%20', \App\Photo::find($category_price->photo_id)->file))}}" alt="product image">
                    </div>
                </div>
                <div class="col-md-7 col-xs-12">
                    <div class="info_pro">
                        <h3>{{ app()->getLocale() =='ar' ? $category_price->name : $category_price->name_en}}</h3>
                        @if($category_price->offer_price == null)
                      <p class="price">{{$category_price->final_price}}</p>
                       @if($show_cart)
                       @if($category_price->final_price)
                      <a href="#" data-name="{{app()->getLocale() =='ar' ? $category_price->name :$category_price->name_en}}" data-price="{{ $category_price->final_price}}"  data-id="{{ $category_price->id}}"  class="add-to-cart btn btn-primary" ><i class="fas fa-cart-plus"></i>add to cart </a>
                         @endif
                          @endif
                       @else
                        <p class="old_price">{{$category_price->final_price}}</p>
                        <p class="price">{{$category_price->offer_price}}</p>
                          @if($show_cart)
                        <a href="#" data-name="{{app()->getLocale() =='ar' ? $category_price->name :$category_price->name_en}}" data-price="{{ $category_price->offer_price}}"  data-id="{{ $category_price->id}}"  class="add-to-cart btn btn-primary" ><i class="fas fa-cart-plus"></i>add to cart </a>
                         @endif
                        @endif
                        <p class="desk_pro">{{app()->getLocale() =='ar' ?  strip_tags(htmlspecialchars_decode( $category_price->description)) : strip_tags(htmlspecialchars_decode($category_price->description_en))}}</p>
