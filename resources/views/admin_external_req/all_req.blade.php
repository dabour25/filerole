@php
    use Carbon\Carbon;
@endphp
@extends('layouts.admin', ['title' => __('strings.external_req_all')])
@section('styles')
                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-white">
                        <div class="panel-heading clearfix">
                            <h4 class="panel-title"> {{ __('strings.external_req_search') }} </h4>
                        </div>
                            ...
                            <form action="{{ Route('search_cust_req') }}" id="serachExternalReqForm">
                                ...
                                <div class="col-md-4 col-xs-12 form-group">
                                    ...
                                    <select class="form-control" name="cust_id">
                                        ...
                                        @foreach ($customersSelect as $cust)
                                            ...
                                            <option value="{{ $cust->id }}" {{ $oldData && $oldData['cust_id'] ==  $cust->id ? 'selected' :''}}>
                                                {{ app()->getLocale() == 'ar' ? $cust->name : $cust->name_en }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-4 col-xs-12 form-group">
                                    ...
                                    <input type="number" class="form-control" name="invoice_no"
                                           value="{{ $oldData  ? $oldData['invoice_no']  :'' }}">
                                </div>
                                <div class="submitBtnSearch col-md-12  text-left">
                                    <button type="submit"
                                            class="btn btn-primary">{{ __('strings.external_req_search') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="panel panel-white">
                        <div class="panel-heading clearfix">
                            <h4 class="panel-title">  {{ __('strings.external_req_all') }}</h4>
                        </div>
                        <div class="panel-body">
                            <table id="xtreme-table" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tbody>
                                @foreach ($items as $doneReq)
                                    @php
                                        $req_trans = DB::select('SELECT sum(quantity * reg_flag * final_price)   AS price  FROM `external_trans` WHERE  org_id = '.Auth::user()->org_id.' AND cust_id = '.$doneReq->Customer->id .' AND external_req_id = '.$doneReq->id );
                                    ...
                                    <tr>
                                        <td>{{ $doneReq->id }}</td>
                                        <td>{{ $doneReq->invoice_code }}</td>
                                        <td>{{ app()->getLocale() == 'ar' ?  $doneReq->Customer->name : $doneReq->Customer->name_en }}</td>
                                        <td>{{$doneReq->telephone}}</td>
                                        <td>{{ date("$date_temp->value",strtotime($doneReq->request_dt) )   }}</td>
                                        <td>{{ Decimalplace(abs($req_trans[0]->price) + abs($doneReq->delivery_fees)) }}</td>
                                        <td>
                                            @if ($doneReq->confirm == 'd')
                                                {{__('strings.external_req_res_delivDone')}}
                                            @elseif($doneReq->confirm == 'c')
                                                {{ __('strings.external_req_res_cancel') }}
                                            @elseif ($doneReq->confirm == 'y' || $doneReq->confirm == 'yx')
                                                {{ __('strings.external_req_res_done_req') }}
                                            @elseif ($doneReq->confirm == 'n')
                                                {{ __('strings.external_req_res_wait') }}
                                            @elseif ($doneReq->confirm == 'p')
                                                {{ __('strings.external_req_res_basket') }}
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
