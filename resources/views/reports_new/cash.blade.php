...
$users=App\User::join('permission_receiving_payments', function ($join) { $join->on('users.id', '=', 'permission_receiving_payments.user_id')->where([ 'users.org_id' => Auth::user()->org_id, 'is_active'=>1  ]); })->select('name','users.id','name_en')->distinct()->get();
@endphp
